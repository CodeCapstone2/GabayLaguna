      ...
      if (res.ok) {
        ...
        poiData.forEach(poi => {
          if (poi.city) {
            ...
            uniqueCities[poi.city_id] = { id: poi.city_id, name: `City ${poi.city_id}` };
          }
        });
...
const submitApplication = async (e) => {
  ...
  try {
    ...
    if (!res.ok) {
      throw new Error(data.message || `Failed to submit application (${res.status})`);
    }
    ...
    <div className="container py-5">
      ...
      <form onSubmit={submitApplication} className="card p-3 mb-4 shadow-sm border-0">
        <div className="row g-3">
          ...
          <div className="col-md-4">
            <label className="form-label">Select Point of Interest (optional)</label>
            <select 
              className="form-select" 
              name="poiId" 
              value={form.poiId} 
        ...
        <div className="mt-3">
          <button 
            ...
            disabled={submitting || (!form.cityId && !form.poiId && !form.notes)}
          >
          ...
          <div className="table-responsive">
                ...
                {pois.length === 0 ? (
                  ...
                  pois.slice(0, 10).map(poi => (
                    <tr key={poi.id}>
                      ...
                      <td>{poi.name}</td>
                      <td>{poi.city ? poi.city.name : `City ${poi.city_id}`}</td>
                      <td>{poi.category ? poi.category.name : poi.category_id || 'N/A'}</td>
            ...
            {pois.length > 10 && (
              <div className="text-center mt-2">
                <small className="text-muted">
                  Showing 10 of {pois.length} POIs. Use the dropdown above to filter.
                </small>
              </div>
            )}
          </div>
          ...
          <tbody>
            ...
            {applications.map(a => (
              <tr key={a.id}>
                ...
                <td>{a.point_of_interest ? a.point_of_interest.name : (a.poi_id ? getPoiName(a.poi_id) : "All POIs")}</td>
                <td>
                  <span className={`badge ${a.status === 'approved' ? 'bg-success' : a.status === 'rejected' ? 'bg-danger' : 'bg-warning'}`}>
                    {a.status}
                  </span>
                </td>
                <td>{new Date(a.created_at).toLocaleString()}</td>
                <td className="text-truncate" style={{maxWidth: '200px'}} title={a.message}>
                  {a.message || '-'}
                </td>
              </tr>
            ))}
